...
RegisterNetEvent('doj-vehs:replacePoppedTire', function()
			...
			if not IsPedInAnyVehicle(playerPed) then
				...
				if vehicle ~= nil then
					...
					if tire ~= nil then 
						...
						ESX.ShowHelpNotification('Press [E] to replace tire')
						if IsControlJustPressed(1, 38) then
							--exports['textUi']:DrawTextUi('show', "Replacing Tire")
							exports['progressBars']:startUI(5000, 'Replacing Tire')
							ClearPedSecondaryTask(playerPed)
							TaskStartScenarioInPlace(playerPed, 'world_human_gardener_plant', 0, false)
							Wait(5000)
							SetVehicleTyreFixed(vehicle, tire.tireIndex, 0, 1)
							TriggerServerEvent('doj-vehs:removeTire')
							deleteProp()
						end
					else
						--exports['textUi']:DrawTextUi('show', "Move closer to a tire</br>Press [X] put away")
						ESX.ShowHelpNotification('Move closer to a tire. Press [X] put away')
					end
				end
				if IsControlJustPressed(1, 73) then
					--exports['textUi']:DrawTextUi('show', "Putting tire away")
					exports['progressBars']:startUI(3000, 'Putting tire away')
					Wait(500)
					deleteProp()
				end
			end
